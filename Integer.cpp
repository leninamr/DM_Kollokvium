...
void CorrectInput(string s, int& j) {
    ...
    while (j < s.length()) {  // проходимся по строке
        if (s[0] == '-') {    // если первый символ минус
        ...
        else if (s[0]!='0'){  // если первый символ не минус и не 0
            if ((s[j] != '0') && (s[j] != '1') && (s[j] != '2') && (s[j] != '3') && (s[j] != '4') && (s[j] != '5') && (s[j] != '6') && (s[j] != '7') && (s[j] != '8') && (s[j] != '9')) break;
            else j++;
        }
        else {  // если первый символ 0
            j = 1;   // длина числа должна быть равна 1
            break;
        }
    }
}
...
Integer Input2(string s)
{
    ...
    if (j != s.length())   // если кол-во символов в строке не равно кол-ву корректных символов
        do {
            cout << "\nОшибка! Введите корректное целое число: ";   // просим ввести правильно
            cin >> s;
            CorrectInput(s, j);
