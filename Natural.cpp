...
Natural InputNatural()
{
	...
	while (1) {
		...
		{
			a = s[i];
			if (a == 48 && i == 0 && N.length != 1) {
				cout << "Число не может начинаться с нуля, введите корректное значение: ";
				break;
			} //если число не 0, но начинается с 0
			if ((47 < a) && (a < 58))
			{
...
Natural SUB_NDN_N(Natural N, Natural N2, int D)
{
	...
	else cout << "Некорректный ввод. Результат будет отрицательным\n";
	//DeleteNatural(temp2);
	return temp;
}
...
{
	if (NZER_N_B(N) == 1) { cout << "Ошибка. Деление на "; return N; }
	if (NZER_N_B(N2) == 1) { cout << "Ошибка. Деление на "; return N2; }
...
{
	if (NZER_N_B(N) == 1) { cout << "Ошибка. В этом модуле недопустим ввод "; return N; }
	if (NZER_N_B(N2) == 1) { cout << "Ошибка. В этом модуле недопустим ввод "; return N2; }
